<template>
  <div class="product">
    <div>
      ...
      <div><strong>Количество:</strong> {{ application.amount }}</div>
      <div><strong>Период доставки:</strong> {{ application.delivery_time }}</div>
      <div><strong>Период принятия заявок:</strong> {{ application.offer_time }}</div>
    </div>
  ...
  methods: {
    findProductName() {
      axios.post("http://localhost:5000/api/product_names/get_one_product_name", {
        id: this.application.productNameId
      }, {
        headers: {
    ...
    openApplication() {
      this.$store.commit('setApplicationId', this.application.id)
